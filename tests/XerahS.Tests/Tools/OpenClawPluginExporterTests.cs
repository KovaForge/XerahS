    ...
    public async Task Export_CreatesUploadFocusedNativeOpenClawPlugin()
    {
        ...
        try
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(cli, Does.Contain("await printRun(config, [\"upload\", \"--text\", String(text), \"--name\", name, \"--json\"], requireUploadUrl);"));
                ...
                Assert.That(cli, Does.Contain("[\"upload\", \"--text\", String(text), \"--name\", name, \"--json\"]"));
