...
/// <see cref="PulseAudioHelper.RunPactlCapture"/>. The fix mirrors the
/// LinuxScreenService v0.23.91 and LinuxThemeService v0.23.92 templates:
/// drain stderr asynchronously and bound the stdout read with
/// <see cref="Task.WaitAny(Task[])"/>.
/// </summary>
[TestFixture]
public class PulseAudioHelperTests
{
    [Test]
    public void RunPactlCapture_HappyPath_ExitsZeroAndReturnsStdout()
    {
        ...
        // path through the run helper used by GetDefaultMonitorSource.
        var startUtc = DateTime.UtcNow;
        var (output, exitCode) = PulseAudioHelper.TestAccessor
            .RunPactlCapture("/bin/sh", "-c \"echo alsa_output.pci-0000_00_1f.3.analog-stereo; exit 0\"", 5000);
        ...
        Assert.That(output, Does.Contain("alsa_output"),
    ...
    [Test]
    public void RunPactlCapture_StderrExceedsPipeBuffer_ToolDoesNotBlock()
    {
        if (!OperatingSystem.IsLinux() && !OperatingSystem.IsMacOS())
        {
            Assert.Ignore("POSIX-specific test (Linux/macOS).");
    ...
    [Test]
    public void RunPactlCapture_TimeoutReturnsNullExitCode()
    {
        if (!OperatingSystem.IsLinux() && !OperatingSystem.IsMacOS())
        {
            Assert.Ignore("POSIX-specific test (Linux/macOS).");
        ...
        var startUtc = DateTime.UtcNow;
        var (_, exitCode) = PulseAudioHelper.TestAccessor
            .RunPactlCapture("/bin/sh", $"-c \"sleep {sleepSeconds}; exit 0\"", timeoutMs);
        var elapsedMs = (DateTime.UtcNow - startUtc).TotalMilliseconds;
    ...
    [Test]
    public void RunPactlCapture_NonZeroExitCode_PropagatesExitCode()
    {
        if (!OperatingSystem.IsLinux() && !OperatingSystem.IsMacOS())
        {
            Assert.Ignore("POSIX-specific test (Linux/macOS).");
        ...
        // Exit with a non-zero code and a small stderr message. The helper
        // should propagate the exit code (exitCode == 7), not report null.
        const int expectedExit = 7;

        var startUtc = DateTime.UtcNow;
        var (_, exitCode) = PulseAudioHelper.TestAccessor
            .RunPactlCapture("/bin/sh", $"-c \"echo warning 1>&2; exit {expectedExit}\"", 5000);
        var elapsedMs = (DateTime.UtcNow - startUtc).TotalMilliseconds;
