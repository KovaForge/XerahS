...
[TestFixture]
public class LinuxThemeServiceTests
{
    [Test]
    public void RunGsettingsCapture_HappyPath_ExitsZeroAndReturnsStdout()
    {
        ...
        // path through the run helper used by TryReadFromGSettings and
        // TryReadGtkThemeDark.
        var startUtc = DateTime.UtcNow;
        var (output, exitCode) = LinuxThemeService.TestAccessor
            .RunGsettingsCapture("/bin/sh", "-c \"echo prefer-dark; exit 0\"", 5000);
        ...
        Assert.That(output, Does.Contain("prefer-dark"),
    ...
    [Test]
    public void RunGsettingsCapture_StderrExceedsPipeBuffer_ToolDoesNotBlock()
    {
        if (!OperatingSystem.IsLinux() && !OperatingSystem.IsMacOS())
        {
            Assert.Ignore("POSIX-specific test (Linux/macOS).");
    ...
    [Test]
    public void RunGsettingsCapture_TimeoutReturnsNullExitCode()
    {
        if (!OperatingSystem.IsLinux() && !OperatingSystem.IsMacOS())
        {
            Assert.Ignore("POSIX-specific test (Linux/macOS).");
    ...
    [Test]
    public void RunGsettingsCapture_NonZeroExit_ReturnsExitCode()
    {
        if (!OperatingSystem.IsLinux() && !OperatingSystem.IsMacOS())
        {
            Assert.Ignore("POSIX-specific test (Linux/macOS).");
        ...
        var (output, exitCode) = LinuxThemeService.TestAccessor
            .RunGsettingsCapture("/bin/sh", "-c \"echo noise 1>&2; exit 7\"", 5000);
