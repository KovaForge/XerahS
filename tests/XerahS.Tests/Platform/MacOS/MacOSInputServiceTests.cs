...
/// <see cref="MacOSInputService.RunOsaScriptCapture"/>. The fix mirrors the
/// LinuxScreenService v0.23.91, LinuxThemeService v0.23.92,
/// PulseAudioHelper v0.23.93, and LinuxInputService v0.23.94 templates:
/// drain stderr asynchronously and bound the stdout read with
/// <see cref="Task.WaitAny(Task[])"/>.
/// </summary>
[TestFixture]
public class MacOSInputServiceTests
{
    [Test]
    public void RunOsaScriptCapture_HappyPath_ExitsZeroAndReturnsStdout()
    {
        ...
        // Mimic osascript's coordinate output and exit 0. Verifies the
        // happy path through the run helper used by
        // MacOSInputService.GetCursorPosition.
        var startUtc = DateTime.UtcNow;
        var (output, exitCode) = MacOSInputService.TestAccessor
            .RunOsaScriptCapture("/bin/sh", "-c \"printf '1234, 567'; exit 0\"", 5000);
        ...
        Assert.That(output, Does.Contain("1234, 567"),
    ...
    [Test]
    public void RunOsaScriptCapture_StderrExceedsPipeBuffer_ToolDoesNotBlock()
    {
        if (!OperatingSystem.IsLinux() && !OperatingSystem.IsMacOS())
        {
            Assert.Ignore("POSIX-specific test (Linux/macOS).");
    ...
    [Test]
    public void RunOsaScriptCapture_TimeoutReturnsNullExitCode()
    {
        if (!OperatingSystem.IsLinux() && !OperatingSystem.IsMacOS())
        {
            Assert.Ignore("POSIX-specific test (Linux/macOS).");
        ...
        var startUtc = DateTime.UtcNow;
        var (_, exitCode) = MacOSInputService.TestAccessor
            .RunOsaScriptCapture("/bin/sh", $"-c \"sleep {sleepSeconds}; exit 0\"", timeoutMs);
        var elapsedMs = (DateTime.UtcNow - startUtc).TotalMilliseconds;
    ...
    [Test]
    public void RunOsaScriptCapture_NonZeroExitCode_PropagatesExitCode()
    {
        if (!OperatingSystem.IsLinux() && !OperatingSystem.IsMacOS())
        {
            Assert.Ignore("POSIX-specific test (Linux/macOS).");
        ...
        // Exit with a non-zero code (osascript returns non-zero on
        // permission failures) and a small stderr message. The helper
        // should propagate the exit code, not report null. This matches
        // the GetCursorPosition contract that `exitCode != 0` returns
        // Point.Empty without throwing.
        const int expectedExit = 1;

        var startUtc = DateTime.UtcNow;
        var (_, exitCode) = MacOSInputService.TestAccessor
            .RunOsaScriptCapture("/bin/sh", $"-c \"echo 'no access' 1>&2; exit {expectedExit}\"", 5000);
        var elapsedMs = (DateTime.UtcNow - startUtc).TotalMilliseconds;
