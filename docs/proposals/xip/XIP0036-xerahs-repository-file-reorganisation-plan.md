...
**Desktop stack** ΓÇö under `src/desktop/`: core/, app/, cli/, tools/, plugins/. **Platform** (shared by desktop and mobile-experimental) is under `src/platform/` (sibling to desktop), not under desktop, so that XerahS.Platform.Mobile is not under a "desktop" path.
...
**Experimental .NET mobile** ΓÇö under **src/mobile-experimental/** (sibling to desktop and mobile):
...
**Native mobile (separate stack)** ΓÇö under **src/mobile/** so one path gives context for both platforms:
...
- `src/mobile/android/` ΓÇö Kotlin app (app/, core/, feature/, build.gradle.kts, etc.)
- `src/mobile/ios/` ΓÇö Swift app
...
Γö£ΓöÇΓöÇ .github/
Γö£ΓöÇΓöÇ .githooks/
Γö£ΓöÇΓöÇ .cursorrules
Γö£ΓöÇΓöÇ .gitignore
Γö£ΓöÇΓöÇ AGENTS.md
Γö£ΓöÇΓöÇ Directory.Build.props
Γö£ΓöÇΓöÇ README.md
Γö£ΓöÇΓöÇ ImageEditor/                    (unchanged; submodule)
Γöé   Γö£ΓöÇΓöÇ src/
Γöé   Γöé   Γö£ΓöÇΓöÇ ShareX.ImageEditor/
Γöé   Γöé   ΓööΓöÇΓöÇ ShareX.ImageEditor.Loader/
Γöé   ΓööΓöÇΓöÇ ShareX.ImageEditor.sln
Γö£ΓöÇΓöÇ docs/
...
Γöé
ΓööΓöÇΓöÇ src/
    Γö£ΓöÇΓöÇ desktop/                     (.NET desktop stack)
    Γöé   Γö£ΓöÇΓöÇ XerahS.sln
    Γöé   Γö£ΓöÇΓöÇ core/
    Γöé   Γöé   Γö£ΓöÇΓöÇ XerahS.Core/
    Γöé   Γöé   Γö£ΓöÇΓöÇ XerahS.Common/
    Γöé   Γöé   Γö£ΓöÇΓöÇ XerahS.Services.Abstractions/
    Γöé   Γöé   Γö£ΓöÇΓöÇ XerahS.Services/
    Γöé   Γöé   Γö£ΓöÇΓöÇ XerahS.ViewModels/
    Γöé   Γöé   Γö£ΓöÇΓöÇ XerahS.History/
    Γöé   Γöé   Γö£ΓöÇΓöÇ XerahS.Indexer/
    Γöé   Γöé   Γö£ΓöÇΓöÇ XerahS.Uploaders/
    Γöé   Γöé   ΓööΓöÇΓöÇ XerahS.Media/
    Γöé   Γö£ΓöÇΓöÇ app/                     (desktop application)
    Γöé   Γöé   Γö£ΓöÇΓöÇ XerahS.App/
    Γöé   Γöé   Γö£ΓöÇΓöÇ XerahS.UI/
    Γöé   Γöé   Γö£ΓöÇΓöÇ XerahS.Bootstrap/
    Γöé   Γöé   ΓööΓöÇΓöÇ XerahS.RegionCapture/
    Γöé   Γö£ΓöÇΓöÇ cli/                     (CLI product ΓÇö shares core, sibling to app)
    Γöé   Γöé   ΓööΓöÇΓöÇ XerahS.CLI/
    Γöé   Γö£ΓöÇΓöÇ tools/
    Γöé   Γöé   Γö£ΓöÇΓöÇ XerahS.WatchFolder.Daemon/
    Γöé   Γöé   Γö£ΓöÇΓöÇ XerahS.PluginExporter/
    Γöé   Γöé   ΓööΓöÇΓöÇ XerahS.Audits.Tool/
    Γöé   Γö£ΓöÇΓöÇ plugins/
    Γöé   Γöé   Γö£ΓöÇΓöÇ AmazonS3.Plugin/
    Γöé   Γöé   Γö£ΓöÇΓöÇ Imgur.Plugin/
    Γöé   Γöé   Γö£ΓöÇΓöÇ Paste2.Plugin/
    Γöé   Γöé   Γö£ΓöÇΓöÇ GitHubGist.Plugin/
    Γöé       Γöé   ΓööΓöÇΓöÇ Auto.Plugin/
    Γö£ΓöÇΓöÇ platform/                    (shared by desktop and mobile-experimental)
    Γöé   Γö£ΓöÇΓöÇ XerahS.Platform.Abstractions/
    Γöé   Γö£ΓöÇΓöÇ XerahS.Platform.Windows/
    Γöé   Γö£ΓöÇΓöÇ XerahS.Platform.Linux/
    Γöé   Γö£ΓöÇΓöÇ XerahS.Platform.MacOS/
    Γöé   ΓööΓöÇΓöÇ XerahS.Platform.Mobile/
    Γö£ΓöÇΓöÇ mobile-experimental/       (experimental .NET mobile ΓÇö Ava, Maui, etc.)
    Γöé   Γö£ΓöÇΓöÇ XerahS.Mobile.Core/
    Γöé   Γö£ΓöÇΓöÇ XerahS.Mobile.Ava/
    Γöé   Γö£ΓöÇΓöÇ XerahS.Mobile.Maui/
    Γöé   ΓööΓöÇΓöÇ XerahS.Mobile.iOS.ShareExtension/
    Γöé
    ΓööΓöÇΓöÇ mobile/                    (native apps ΓÇö one folder = both platforms)
        Γö£ΓöÇΓöÇ android/                (Kotlin; ex XerahS.Mobile.Kt)
        Γöé   Γö£ΓöÇΓöÇ app/
        Γöé   Γö£ΓöÇΓöÇ core/
        Γöé   Γöé   Γö£ΓöÇΓöÇ common/
        Γöé   Γöé   Γö£ΓöÇΓöÇ data/
        Γöé   Γöé   ΓööΓöÇΓöÇ domain/
        Γöé   Γö£ΓöÇΓöÇ feature/
        Γöé   Γöé   Γö£ΓöÇΓöÇ upload/
        Γöé   Γöé   Γö£ΓöÇΓöÇ history/
        Γöé   Γöé   ΓööΓöÇΓöÇ settings/
        Γöé   Γö£ΓöÇΓöÇ build.gradle.kts
        Γöé   ΓööΓöÇΓöÇ settings.gradle.kts
        ΓööΓöÇΓöÇ ios/                    (Swift; ex XerahS.Mobile.Swift)
            ΓööΓöÇΓöÇ ...
```
...
**Stage 4 mini-stages (one commit per core project)** ΓÇö Create `src/desktop/core/` when moving the first project. For each row: move that project into `src/desktop/core/`, update .sln path for it, update every .csproj that references it (app, cli, mobile-experimental, tools, plugins, other core projects), then commit.
...
**Stage 5 mini-stages (one commit per platform project)** ΓÇö Create `src/desktop/platform/` when moving the first project. For each row: move that project into `src/desktop/platform/`, update .sln path, update every .csproj that references it, then commit.
...
**Stage 7 mini-stages (one commit per tools project)** ΓÇö Create `src/desktop/tools/` when moving the first project. For each row: move that project into `src/desktop/tools/`, update .sln path, update every .csproj that references it, then commit.
...
**Stage 8 mini-stages (one commit per plugin, move + shorten path)** ΓÇö Create `src/desktop/plugins/` when moving the first plugin. For each row: move that plugin from `src/Plugins/ShareX.X.Plugin` to `src/desktop/plugins/X.Plugin` (drop ShareX. prefix so path is shorter, e.g. AmazonS3.Plugin, Imgur.Plugin). Update .sln project path and any solution folder display name (e.g. ShareX.AmazonS3.Plugin ΓåÆ AmazonS3.Plugin), update XerahS.App _DefaultPlugins, mobile-experimental ProjectReferences if they reference that plugin, and the pluginΓÇÖs own ProjectReferences to core (`..\..\core\...`). Then commit.
...
**Status:** Stages 5aΓÇô5e (platform), 6aΓÇô6d (app), 7aΓÇô7c (tools), 8aΓÇô8e (plugins, short paths) completed ΓÇö one commit per mini-stage.
...
7. `git add` ΓåÆ `git commit` with the stage message above.
...
- **After Stage 3**: CLI at `src/desktop/cli/XerahS.CLI/`. CLIΓÇÖs ProjectReferences: `..\..\..\XerahS.Core\` etc. (three levels up to src; core still flat).
- **After Stage 4 (4aΓÇô4i)**: Each core project moved one at a time. After 4a: only XerahS.Core is under `src/desktop/core/`; referrers use `..\..\core\XerahS.Core\` (from app/cli) or `..\..\..\core\XerahS.Core\` (from mobile-experimental) or `..\core\XerahS.Core\` (from other core projects). Repeat for 4bΓÇô4i; by 4i all core is at `src/desktop/core/`.
- **Stages 5ΓÇô8**: Same idea: update .sln and every ProjectReference that points to the moved projects. ImageEditor path from UI (Stage 6): `..\..\..\..\ImageEditor\src\ShareX.ImageEditor\ShareX.ImageEditor.csproj`.
...
**2.** **Move Core projects** ΓÇö `git mv` into `src/desktop/core/`: XerahS.Core, XerahS.Common, XerahS.Services.Abstractions, XerahS.Services, XerahS.ViewModels, XerahS.History, XerahS.Indexer, XerahS.Uploaders, XerahS.Media.

**3.** **Move Platform projects** ΓÇö `git mv` into `src/desktop/platform/`: XerahS.Platform.Abstractions, XerahS.Platform.Windows, XerahS.Platform.Linux, XerahS.Platform.MacOS, XerahS.Platform.Mobile.

**4.** **Move desktop app** ΓÇö `git mv` into `src/desktop/app/`: XerahS.App, XerahS.UI, XerahS.Bootstrap, XerahS.RegionCapture (the application; **not** a folder named desktop).

**5.** **Move CLI** ΓÇö `git mv src/XerahS.CLI src/desktop/cli/XerahS.CLI` (CLI shares core; product under desktop, sibling to app).

**6.** **Move Kotlin app** ΓÇö `git mv src/XerahS.Mobile.Kt src/mobile/android` (folder name **android**, not XerahS.Mobile.Kt).

**7.** **Move Swift app** ΓÇö `git mv src/XerahS.Mobile.Swift src/mobile/ios` (folder name **ios**, not XerahS.Mobile.Swift).

**8.** **Move experimental mobile** ΓÇö `git mv` into `src/mobile-experimental/`: XerahS.Mobile.Core, XerahS.Mobile.Ava, XerahS.Mobile.Maui, XerahS.Mobile.iOS.ShareExtension.

**9.** **Move Tools projects** ΓÇö `git mv` into `src/desktop/tools/`: XerahS.WatchFolder.Daemon, XerahS.PluginExporter, XerahS.Audits.Tool (XerahS.CLI already moved to src/desktop/cli/ in step 5).

**10.** **Move Plugins** ΓÇö For each plugin: move `src/Plugins/ShareX.X.Plugin` to `src/desktop/plugins/X.Plugin` (drop ShareX. prefix for shorter path). Update solution and ProjectReferences (steps 11 and 12).
...
**14.** **Plugin folder names** ΓÇö Under `src/desktop/plugins/`, use short folder names (e.g. AmazonS3.Plugin, Imgur.Plugin) by dropping the ShareX. prefix when moving. Update solution and all ProjectReferences accordingly.

**15.** **Update build-windows-exe SKILL.md** ΓÇö Update hardcoded paths to `src\desktop\...`; ImageEditor paths unchanged.

**16.** **Update build-android SKILL.md** ΓÇö Update paths to `src\mobile\android\` for Kotlin app; `src\mobile-experimental\...` if MAUI/Avalonia mobile referenced; plugin paths if step 14 done.

**17.** **Update build-linux-binary SKILL.md** ΓÇö Update any `src/` paths to `src/desktop/...`, `src/mobile-experimental/...`, `src/mobile/android/`, `src/mobile/ios/`.

**18.** **Update other scripts/docs** ΓÇö Search `.githooks/`, `scripts/`, docs for old `src\XerahS.*` or `src\Plugins` paths and fix to `src\desktop\...`, `src\mobile-experimental\...`, `src\mobile\android\`, `src\mobile\ios\`.

**19.** **Optional: rename test folder** ΓÇö `tests/ShareX.Avalonia.Tests` ΓåÆ `tests/XerahS.Avalonia.Tests` and update solution + references.

**20.** **Verify** ΓÇö Run `dotnet build src/desktop/XerahS.sln` from repo root (0 errors); build Android/iOS if in use.
...
- **Plugin folder names**: Stage 8 moves and renames in one go: ShareX.X.Plugin ΓåÆ src/desktop/plugins/X.Plugin (shorter path). Update every ProjectReference and the solution.
...
- Or **only rename** plugin directories ShareX.* ΓåÆ XerahS.* and optionally the test folder; no moves.
