...
## Legacy content from `XIP0007-aftercapturejobs-upload-processor-backend.md`
...
## Legacy content from `XIP0007_AfterCapture_Upload.md`
...
## Objective
Implement `UploadImageToHost` in `CaptureJobProcessor` to enable automatic image uploads after capture. This completes the ShareX workflow: **Capture ΓåÆ AfterCapture ΓåÆ Upload ΓåÆ AfterUpload**.

## Background
Currently, only 3/21 AfterCaptureTasks work:
- Γ£à SaveImageToFile
- Γ£à CopyImageToClipboard
- Γ£à AnnotateImage
- Γ¥î **UploadImageToHost** ΓåÉ Your task
...
## Deliverables
- Γ£à `CaptureJobProcessor.cs` updated with UploadImageToHost handler
- Γ£à Build succeeds on `feature/after-capture-upload`
- Γ£à Debug logs show upload attempt when flag is set
