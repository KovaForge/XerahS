...
## Legacy content from `XIP0019-platform-abstraction-architecture-audit.md`
...
## Legacy content from `XIP0019_Platform_Abstraction_Audit.md`
...
- **Rule**: Eliminate hardcoded paths or separators.
    - Γ¥î `C:\Program Files\`
    - Γ¥î `\\` (Backslash hardcoding) -> Use `Path.Combine()` or `Path.DirectorySeparatorChar`
    - Γ¥î `\r\n` (CRLF) for logic -> Use `Environment.NewLine`

### 3.3 Static Gateway Violations
- **Rule**: No static access to platform services in Core logic.
    - Γ¥î `Clipboard.SetText(...)`
    - Γ£à `IClipboardService.SetText(...)` (Injected)
    - Γ¥î `MessageBox.Show(...)`
    - Γ£à `IDialogService.ShowMessage(...)`
