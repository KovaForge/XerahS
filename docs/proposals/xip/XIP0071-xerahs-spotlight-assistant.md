...
| MiniMax | Dedicated MiniMax adapter; prefer the documented compatible API path that best matches tool calling after implementation validation | MiniMax API key | `MiniMax-Text-01` |
...
| Gemini | Dedicated Gemini adapter using `generateContent`, function declarations, and function calling config | `x-goog-api-key` | `gemini-1.5-flash` |
| Anthropic | Dedicated Anthropic Messages adapter using client-side tools and tool-result loop | `x-api-key` plus `anthropic-version` | `claude-3-haiku-20240307` |
