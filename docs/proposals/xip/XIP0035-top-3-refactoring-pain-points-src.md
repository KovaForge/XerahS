...
## Pain Point 1: God class ΓÇô `LinuxScreenCaptureService` (~2,146 lines) ΓÇö **Refactored (XIP0035)**
...
- `Capture/Helpers/LinuxCliToolRunner.cs` ΓÇô run screenshot CLI tools (testable).
- `Capture/X11/X11ScreenCapture.cs` ΓÇô X11 XGetImage fullscreen capture.
- `Capture/Portal/PortalScreenCapture.cs` ΓÇô XDG Portal screenshot (D-Bus).
- `Capture/Kde/KdeDbusScreenCapture.cs` ΓÇô KDE ScreenShot2 D-Bus.
- `Capture/Gnome/GnomeDbusScreenCapture.cs` ΓÇô GNOME Shell Screenshot D-Bus.
- `Capture/Wayland/WaylandCliCapture.cs` ΓÇô grim, slurp, grimblast, hyprshot.
- `Capture/Cli/CliCaptureExecutor.cs` ΓÇô gnome-screenshot, spectacle, scrot, import, xfce4-screenshooter.
...
## Pain Point 2: Static service locator and no dependency injection ΓÇö **Refactored (XIP0035)**
...
- **Composition root:** `src/desktop/app/XerahS.UI/Services/CompositionRoot.cs` ΓÇô builds the service provider from current `PlatformServices` state after platform init and after UI/Toast/ImageEncoder are registered; called from `App.axaml.cs` `OnFrameworkInitializationCompleted`.
...
## Pain Point 3: Duplicate mobile config ViewModels (Mobile.UI vs Mobile.Maui) ΓÇö **In scope for this XIP**
