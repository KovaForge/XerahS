...
## Pain Point 1: God ViewModels ΓÇö `TaskSettingsViewModel` (1,403 lines) & `SettingsViewModel` (1,293 lines) Γ£à REFACTORED
...
**TaskSettingsViewModel** (1,403 ΓåÆ 9 files):
| File | Contents | Lines |
|------|----------|-------|
...
**SettingsViewModel** (1,293 ΓåÆ 7 files):
| File | Contents | Lines |
|------|----------|-------|
...
## Pain Point 2: Platform Service Duplication ΓÇö Watch Folder Daemon (~1,700 lines across 3 files) Γ£à REFACTORED
...
- `RestartAsync` ΓÇö concrete implementation (stop then start), previously copy-pasted in all 3 files
- `ResolveDaemonPath` ΓÇö unified daemon path resolution with platform-specific executable candidate arrays
- `RunProcessAsync` ΓÇö core process runner (captures stdout/stderr, handles timeout), replaces 3 identical implementations
- `RunProcessWithArgumentsAsync` ΓÇö variant using ArgumentList for safe argument passing
- `EscapeShellSingleQuotedString` ΓÇö shared by macOS and Linux
- `RunPrivilegedShellScriptAsync` ΓÇö temp-file-based privileged script execution pattern shared by macOS and Linux
- `CommandResult` ΓÇö unified record struct replacing `ScCommandResult`, `LaunchCtlResult`, `SystemCommandResult`

**Line count changes:**
| File | Before | After | Saved |
|------|--------|-------|-------|
| `WatchFolderDaemonServiceBase.cs` (new) | ΓÇö | ~200 | ΓÇö |
...
## Pain Point 3: `ScreenRecordingManager` ΓÇö Duplicated Logic & State Complexity (1,003 lines) Γ£à REFACTORED
...
1. **Extracted `StartRecordingCoreAsync`** ΓÇö The 2-attempt recording loop (lock ΓåÆ create service ΓåÆ wire events ΓåÆ start ΓåÆ fallback on failure) was copy-pasted between `StartRecordingAsync` and `StartRecordingInternalAsync`. Now both call a single shared `StartRecordingCoreAsync(optionsToStart, preferFallback)` method.

2. **Removed 14 `Console.WriteLine` calls** ΓÇö Each had a proper `DebugHelper.WriteLine` or `TroubleshootingHelper.Log` call right next to it, making them purely redundant debug spam.

3. **Simplified `StartRecordingInternalAsync`** ΓÇö Reduced from ~100 lines to 6 lines (init check + fallback decision + prepare options + delegate to core).

**Line count:** 1,003 ΓåÆ ~870 lines (~130 lines removed)
...
|---|---|---|---|---|---|
| 1. God ViewModels | **Critical** (most-touched files) | High | 2-3 days | P0 | Γ£à Done |
| 2. Platform duplication | High | **Critical** (bug propagation) | 2-3 days | P1 | Γ£à Done |
| 3. ScreenRecordingManager | Medium | High (deadlock risk) | 1-2 days | P2 | Γ£à Done |