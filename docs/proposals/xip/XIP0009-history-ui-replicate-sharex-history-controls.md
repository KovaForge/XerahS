...
<Border Grid.Row="0" Padding="12,8">
    <Grid ColumnDefinitions="Auto,*,Auto">
        ...
        <StackPanel Grid.Column="0" Orientation="Horizontal" Spacing="8">
            ...
            <ToggleButton Content="??" IsChecked="{Binding IsAdvancedSearchVisible}" 
...
<Border.ContextMenu>
    <ContextMenu>
        ...
        <MenuItem Header="Open">
            ...
            <MenuItem Header="??? Thumbnail URL" Command="{Binding ...OpenThumbnailURLCommand}" CommandParameter="{Binding}"/>
            <MenuItem Header="??? Deletion URL" Command="{Binding ...OpenDeletionURLCommand}" CommandParameter="{Binding}"/>
            <Separator/>
        ...
        <MenuItem Header="Copy">
            ...
            <MenuItem Header="??? Thumbnail URL" Command="{Binding ...CopyThumbnailURLCommand}" CommandParameter="{Binding}"/>
            <MenuItem Header="??? Deletion URL" Command="{Binding ...CopyDeletionURLCommand}" CommandParameter="{Binding}"/>
            <Separator/>
            <MenuItem Header="?? File" Command="{Binding ...CopyFileCommand}" CommandParameter="{Binding}" InputGesture="Shift+C"/>
            <MenuItem Header="??? Image" Command="{Binding ...CopyImageCommand}" CommandParameter="{Binding}" InputGesture="Alt+C"/>
        ...
        <MenuItem Header="??? Edit tag..." Command="{Binding ...EditTagCommand}" CommandParameter="{Binding}"/>
        <MenuItem Header="?? Edit item..." Command="{Binding ...EditItemCommand}" CommandParameter="{Binding}"/>
        <MenuItem Header="?? Rename file..." Command="{Binding ...RenameFileCommand}" CommandParameter="{Binding}"/>
        <MenuItem Header="??? Delete item..." Command="{Binding ...DeleteItemCommand}" CommandParameter="{Binding}" InputGesture="Delete"/>
        <MenuItem Header="??? Delete file &amp;&amp; item..." Command="{Binding ...DeleteFileAndItemCommand}" CommandParameter="{Binding}" InputGesture="Shift+Delete"/>
        
        <Separator/>
        
        <!-- Image Actions -->
        <MenuItem Header="??? Image preview" Command="{Binding ...ShowImagePreviewCommand}" CommandParameter="{Binding}"/>
        <MenuItem Header="?? Upload file" Command="{Binding ...UploadFileCommand}" CommandParameter="{Binding}" InputGesture="Ctrl+U"/>
        <MenuItem Header="?? Edit image" Command="{Binding ...EditImageCommand}" CommandParameter="{Binding}" InputGesture="Ctrl+E"/>
...
## Legacy content from `XIP0009-history-ui-replicate-sharex-history-controls.md`
...
## Legacy content from `XIP0009_History_UI.md`
...
<Border Grid.Row="0" Padding="12,8">
    <Grid ColumnDefinitions="Auto,*,Auto">
        <!-- Left: Search -->
        <StackPanel Grid.Column="0" Orientation="Horizontal" Spacing="8">
            <TextBlock Text="≡ƒöì Search:" VerticalAlignment="Center" Foreground="{DynamicResource SystemControlForegroundBaseMediumBrush}"/>
            <TextBox Text="{Binding SearchText}" 
                     Watermark="Search by filename..."
                     Width="200"
                     CornerRadius="6"
                     Padding="8,6"/>
            <Button Content="≡ƒöÄ" Command="{Binding SearchCommand}" 
                    ToolTip.Tip="Search" Padding="8" CornerRadius="6"/>
            <ToggleButton Content="ΓÜÖ∩╕Å" IsChecked="{Binding IsAdvancedSearchVisible}" 
                          ToolTip.Tip="Advanced Search" Padding="8" CornerRadius="6"/>
        </StackPanel>
        
        <!-- Center: View Controls -->
        <StackPanel Grid.Column="1" Orientation="Horizontal" Spacing="8" HorizontalAlignment="Center">
            <ToggleButton Content="Γ¡É" IsChecked="{Binding ShowFavoritesOnly}" 
                          ToolTip.Tip="Show Favorites Only" Padding="8" CornerRadius="6"/>
            <Button Content="≡ƒôè" Command="{Binding ShowStatsCommand}" 
                    ToolTip.Tip="Show Statistics" Padding="8" CornerRadius="6"/>
            <Button Content="≡ƒôü" Command="{Binding ImportFolderCommand}" 
                    ToolTip.Tip="Import Folder" Padding="8" CornerRadius="6"/>
        </StackPanel>
        
        <!-- Right: Actions -->
        <StackPanel Grid.Column="2" Orientation="Horizontal" Spacing="8">
            <Button Content="≡ƒöä" Command="{Binding RefreshHistoryCommand}" 
                    ToolTip.Tip="Refresh (F5)" Padding="8" CornerRadius="6"/>
            <Button Content="{Binding IsGridView, Converter={x:Static vm:HistoryViewModel.ViewToggleConverter}}" 
                    Command="{Binding ToggleViewCommand}"
                    ToolTip.Tip="Toggle View" Padding="8" CornerRadius="6"/>
...
<Border Grid.Row="1" 
    ...
    <Grid ColumnDefinitions="Auto,*,Auto,*,Auto,*" RowDefinitions="Auto,Auto" RowSpacing="8" ColumnSpacing="12">
        ...
        <Button Grid.Row="0" Grid.Column="4" Content="Reset" Command="{Binding ResetFiltersCommand}" Padding="12,6" CornerRadius="4"/>
        <Button Grid.Row="0" Grid.Column="5" Content="Γ£ò Close" Command="{Binding CloseAdvancedSearchCommand}" Padding="12,6" CornerRadius="4"/>
...
<Border.ContextMenu>
    <ContextMenu>
        <!-- Open Section -->
        <MenuItem Header="Open">
            <MenuItem Header="≡ƒîÉ URL" Command="{Binding $parent[UserControl].((vm:HistoryViewModel)DataContext).OpenURLCommand}"
                      CommandParameter="{Binding}" InputGesture="Enter"/>
            <MenuItem Header="≡ƒöù Shortened URL" Command="{Binding ...OpenShortenedURLCommand}" CommandParameter="{Binding}"/>
            <MenuItem Header="≡ƒû╝∩╕Å Thumbnail URL" Command="{Binding ...OpenThumbnailURLCommand}" CommandParameter="{Binding}"/>
            <MenuItem Header="≡ƒùæ∩╕Å Deletion URL" Command="{Binding ...OpenDeletionURLCommand}" CommandParameter="{Binding}"/>
            <Separator/>
            <MenuItem Header="≡ƒôä File" Command="{Binding ...OpenFileCommand}" CommandParameter="{Binding}" InputGesture="Ctrl+Enter"/>
            <MenuItem Header="≡ƒôé Folder" Command="{Binding ...OpenFolderCommand}" CommandParameter="{Binding}" InputGesture="Shift+Enter"/>
        </MenuItem>
        
        <!-- Copy Section -->
        <MenuItem Header="Copy">
            <MenuItem Header="≡ƒîÉ URL" Command="{Binding ...CopyURLCommand}" CommandParameter="{Binding}" InputGesture="Ctrl+C"/>
            <MenuItem Header="≡ƒöù Shortened URL" Command="{Binding ...CopyShortenedURLCommand}" CommandParameter="{Binding}"/>
            <MenuItem Header="≡ƒû╝∩╕Å Thumbnail URL" Command="{Binding ...CopyThumbnailURLCommand}" CommandParameter="{Binding}"/>
            <MenuItem Header="≡ƒùæ∩╕Å Deletion URL" Command="{Binding ...CopyDeletionURLCommand}" CommandParameter="{Binding}"/>
            <Separator/>
            <MenuItem Header="≡ƒôä File" Command="{Binding ...CopyFileCommand}" CommandParameter="{Binding}" InputGesture="Shift+C"/>
            <MenuItem Header="≡ƒû╝∩╕Å Image" Command="{Binding ...CopyImageCommand}" CommandParameter="{Binding}" InputGesture="Alt+C"/>
            <MenuItem Header="≡ƒô¥ Text" Command="{Binding ...CopyTextCommand}" CommandParameter="{Binding}"/>
            <Separator/>
            <MenuItem Header="HTML link" Command="{Binding ...CopyHTMLLinkCommand}" CommandParameter="{Binding}"/>
            <MenuItem Header="HTML image" Command="{Binding ...CopyHTMLImageCommand}" CommandParameter="{Binding}"/>
            <MenuItem Header="HTML linked image" Command="{Binding ...CopyHTMLLinkedImageCommand}" CommandParameter="{Binding}"/>
            ...
            <MenuItem Header="Markdown linked image" Command="{Binding ...CopyMarkdownLinkedImageCommand}" CommandParameter="{Binding}"/>
            <Separator/>
            <MenuItem Header="≡ƒôü File path" Command="{Binding ...CopyFilePathCommand}" CommandParameter="{Binding}" InputGesture="Ctrl+Shift+C"/>
            <MenuItem Header="≡ƒô¥ File name" Command="{Binding ...CopyFileNameCommand}" CommandParameter="{Binding}"/>
            <MenuItem Header="≡ƒô¥ File name with extension" Command="{Binding ...CopyFileNameWithExtensionCommand}" CommandParameter="{Binding}"/>
            <MenuItem Header="≡ƒôé Folder" Command="{Binding ...CopyFolderCommand}" CommandParameter="{Binding}"/>
        </MenuItem>
        
        <Separator/>
        
        <!-- Actions Section -->
        <MenuItem Header="Γ¡É Favorite" Command="{Binding ...ToggleFavoriteCommand}" CommandParameter="{Binding}"/>
        <MenuItem Header="≡ƒÅ╖∩╕Å Edit tag..." Command="{Binding ...EditTagCommand}" CommandParameter="{Binding}"/>
        <MenuItem Header="Γ£Å∩╕Å Edit item..." Command="{Binding ...EditItemCommand}" CommandParameter="{Binding}"/>
        <MenuItem Header="≡ƒô¥ Rename file..." Command="{Binding ...RenameFileCommand}" CommandParameter="{Binding}"/>
        <MenuItem Header="≡ƒùæ∩╕Å Delete item..." Command="{Binding ...DeleteItemCommand}" CommandParameter="{Binding}" InputGesture="Delete"/>
        <MenuItem Header="≡ƒùæ∩╕Å Delete file &amp;&amp; item..." Command="{Binding ...DeleteFileAndItemCommand}" CommandParameter="{Binding}" InputGesture="Shift+Delete"/>
        
        <Separator/>
        
        <!-- Image Actions -->
        <MenuItem Header="≡ƒû╝∩╕Å Image preview" Command="{Binding ...ShowImagePreviewCommand}" CommandParameter="{Binding}"/>
        <MenuItem Header="Γ¼å∩╕Å Upload file" Command="{Binding ...UploadFileCommand}" CommandParameter="{Binding}" InputGesture="Ctrl+U"/>
        <MenuItem Header="Γ£Å∩╕Å Edit image" Command="{Binding ...EditImageCommand}" CommandParameter="{Binding}" InputGesture="Ctrl+E"/>
        <MenuItem Header="≡ƒôî Pin to screen" Command="{Binding ...PinToScreenCommand}" CommandParameter="{Binding}" InputGesture="Ctrl+P"/>
        <MenuItem Header="≡ƒñû Analyze image..." Command="{Binding ...AnalyzeImageCommand}" CommandParameter="{Binding}"/>
    </ContextMenu>
</Border.ContextMenu>
```
...
| Advanced Search toggle | ToolStripButton (checked) | ToggleButton |
| Favorites filter | ToolStripButton (checked) | ToggleButton |
| Show Stats | ToolStripButton | Button ΓåÆ Dialog |
| Import Folder | ToolStripButton | Button ΓåÆ FolderPicker |
| Settings | ToolStripButton | Button ΓåÆ Settings Dialog |
| List View | MyListView (VirtualMode) | DataGrid or ListBox |
| Split container | SplitContainerCustomSplitter | Grid + GridSplitter |
| Thumbnail preview | MyPictureBox | Image control |
| Context menu | ContextMenuStrip | ContextMenu |
...
## Deliverables
- Γ£à `HistoryView.axaml` updated with enhanced toolbar, advanced search, split view
- Γ£à `HistoryViewModel.cs` has all filter properties and commands
- Γ£à Context menu provides full Open/Copy/Edit/Delete functionality
- Γ£à Preview panel shows selected item thumbnail
- Γ£à Status bar displays total/filtered counts
- Γ£à Build succeeds on `feature/history-ui`
- Γ£à Commit and push changes
...
2. **Filter by date** - enable date filter, set range, verify filtering
3. **Favorites** - toggle favorites filter, verify only favorites show
4. **Copy URL** - right-click ΓåÆ Copy ΓåÆ URL, paste to verify
5. **Open folder** - right-click ΓåÆ Open ΓåÆ Folder, verify explorer opens
6. **Delete item** - right-click ΓåÆ Delete item, confirm dialog, verify removal
7. **Keyboard shortcuts** - F5 refreshes, Enter opens, Delete prompts deletion
