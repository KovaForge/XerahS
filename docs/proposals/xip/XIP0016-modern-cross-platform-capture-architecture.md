...
## Legacy content from `XIP0016-modern-cross-platform-capture-architecture.md`
...
## Legacy content from `XIP0016_Modern_Capture_Architecture.md`
...
**Current Implementation Status**: Γ£à Complete
- `WindowsModernCaptureService` implemented using `Vortice.Direct3D11`/`DXGI`.
- Supports hardware-accelerated capture on Windows 10 (17763)+.
- Automatically falls back to GDI+ on older systems or if modern capture fails.
...
**Current Implementation Status**: Γ£à Complete
- Γ£à `MacOSScreenshotService.cs` - CLI-based fallback (complete)
- Γ£à `MacOSScreenCaptureKitService.cs` - Native ScreenCaptureKit (complete)
...
*   The `IScreenCaptureService` interface already exists in `XerahS.Platform.Abstractions`.
*   Each stage will implement/replace a concrete service:
    *   **Stage 1**: `WindowsModernCaptureService` Γ£à Complete
    *   **Stage 2**: `LinuxScreenCaptureService` Γ£à Complete
    *   **Stage 3**: `MacOSScreenCaptureKitService` ≡ƒöä In Progress
        *   Native library: `native/macos/libscreencapturekit_bridge.dylib`
        *   Fallback: `MacOSScreenshotService` (CLI-based) for macOS < 12.3
*   The system will auto-detect the OS and version to select the best available provider, falling back to legacy methods (GDI+/CLI) only when modern APIs are unavailable.
...
| Platform | Primary (Modern=True) | Legacy/Fallback (Modern=False) | Implementation File | Status |
|----------|-----------------------|--------------------------------|---------------------|--------|
| **Windows** | **DXGI Desktop Duplication** / WGC | GDI+ (`CopyFromScreen`) | `WindowsRegionCaptureBackend.cs` | Γ£à Complete |
| **macOS** | **ScreenCaptureKit** (Native) | `screencapture` CLI | `MacOSScreenCaptureKitService.cs` | Γ£à Complete |
| **Linux** | **Wayland/Portal Tools** (Spectacle/Gnome) | X11 XGetImage | `LinuxScreenCaptureService.cs` | Γ£à Complete |
