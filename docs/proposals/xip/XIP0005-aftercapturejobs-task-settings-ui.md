...
- **Use icons if available** (?? for save, ?? for clipboard, ?? for upload)
...
## Legacy content from `XIP0005-aftercapturejobs-task-settings-ui.md`
...
## Legacy content from `XIP0005_AfterCapture_UI.md`
...
**Your job**: Build the UI so users can check/uncheck tasks in Settings ΓåÆ Task Settings.
...
- **Follow XerahS UI patterns** (dark theme, rounded corners, spacing)
- **Make "Upload image to host" prominent** (it's the main feature)
- **Use icons if available** (≡ƒÆ╛ for save, ≡ƒôï for clipboard, Γ¼å∩╕Å for upload)
- **Ensure settings persist** via SettingManager.Settings.DefaultTaskSettings
...
## Deliverables
- Γ£à TaskSettingsView.axaml updated with "After Capture" section
- Γ£à SettingsViewModel.cs has checkbox binding properties
- Γ£à Build succeeds on `feature/after-capture-ui`
- Γ£à Settings persist when toggled
- Γ£à Commit and push changes

## Testing

### Visual Test
1. **Run app** and navigate to Settings ΓåÆ Task Settings
2. **Verify** "After Capture" section displays
3. **Toggle checkboxes** - confirm they update
4. **Restart app** - verify settings persist
