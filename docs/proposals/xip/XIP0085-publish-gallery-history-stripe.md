...
- The destination URL (Imgur, S3, …) is **not** deleted. Unpublish only removes the gallery row.
...
| Unpublish | Confirm, then DELETE gallery row |
...
| DELETE | `/api/v1/items/{id}` | Idempotent Unpublish; a missing row is success |
...
| `gallery_items` | `public` | UUID `id`; `owner_id`; stable `client_item_id`; exact HTTPS `url`; SHA-256 URL fingerprint; optional thumbnail URL; kind; validated leaf filename; generated title; captured/published/updated timestamps; host/content type |
...
- Enable RLS on every table in an exposed schema. `profiles` and `gallery_items` owner policies compare `(select auth.uid())` with `user_id`/`owner_id` for every `SELECT`, `INSERT`, `UPDATE`, and `DELETE`; updates use both `USING` and `WITH CHECK` so ownership cannot be reassigned.
- `anon` receives no profile or gallery privileges. `authenticated` receives only the exact table/column privileges required. New tables are explicitly exposed/granted if needed because Supabase projects no longer guarantee automatic Data API exposure.
...
- Any `SECURITY DEFINER` function lives in `app_private`, pins `search_path`, verifies `auth.uid()` before accepting a user ID, exposes the smallest scalar result, and has `EXECUTE` revoked from `PUBLIC`. Run Supabase security/performance advisors after every schema change.
- Views are `security_invoker=true` or remain private. Authorization never uses user-editable `user_metadata`.
- Apply a restrictive strong-session policy to gallery `SELECT`, `INSERT`, `UPDATE`, and `DELETE`, in addition to owner policies. The stable TOTP path requires `aal2`; an `aal1` owner is redirected to challenge and cannot view or Unpublish until step-up succeeds. Billing lapse does not block read/delete, but authentication strength always applies. Passkeys may satisfy this policy only after the exact Supabase `aal`/`amr` contract is documented and covered by RLS/API tests.
...
Desktop Unpublish calls the API then clears tags. If the API says 404 already gone, still clear local tags.
...
- R2 operations-ledger credentials are separate, write-minimized server secrets scoped to one locked bucket; restore tooling uses a distinct read credential. Neither credential is exposed to browsers/desktops or shared with Cloudflare DNS automation.
...
| Environment | Vercel access | Supabase | Stripe | WebAuthn |
|---|---|---|---|---|
| Production | Public origin; application auth enforced | Dedicated production project | Live mode | RP ID `xerahs.com`, exact origin `https://xerahs.com` |
| Stable staging | Public `staging.xerahs.com`; application auth and WAF enforced; noindex | Dedicated staging project | Test mode with public signed staging webhook | RP ID/origin `staging.xerahs.com` in a separate credential namespace |
| Ephemeral PR Preview | Vercel Authentication; CI uses automation-bypass secret; noindex | Isolated preview branch/project with synthetic data | Signed fixtures only; no inbound Stripe endpoint | Passkeys disabled; no arbitrary `*.vercel.app` origin |
| Development | Local only | Local or dedicated development project | Stripe CLI/test mode | Registered loopback development origin only |
...
- Profile and gallery metadata live for the account lifetime. Unpublish hard-deletes the gallery row and invalidates application caches immediately; it never deletes the destination object.
...
- Account deletion is an idempotent outbox state machine, not one synchronous transaction. `DELETE /api/v1/account` immediately marks the account `deletion_pending`, blocks access, revokes sessions, and returns an operation ID. A worker retries Stripe cancellation, writes the independently retained tombstone, preserves the minimum statutory Customer/Subscription/cancellation mapping, deletes the Auth user, cascades profile/gallery data, and marks completion. Auth deletion cannot run until cancellation succeeded or is durably queued with retry/alert; never orphan a charging Subscription. Target completion is 24 hours and the published maximum is 30 days except legally retained billing records.
- Replicate deletion tombstones and pseudonymous trial-grant hashes to an encrypted, append-only **Cloudflare R2** operations bucket in a separate account/security boundary with Bucket Lock retention. This ledger is outside the production Supabase PITR boundary and is replayed after restore. Application media is never stored in this bucket.
...
- Deleted data may remain in encrypted backups until backup expiry. Restore procedures must replay deletion tombstones before reopening traffic so deleted users/items are not resurrected.
...
Create the pinned `web/` app, local/preview/production environment separation, Supabase migrations/RLS tests, verified email/password, TOTP `aal2`, browser SSR sessions, desktop PKCE handoff, audit events, headers, and CSP. Keep passkeys disabled until the production acceptance test is met.

### Phase 3 — Trial and Stripe Billing

Implement the one-time trial grant, Stripe Product/Prices, hosted Checkout, Portal configuration, signed/idempotent webhook processing, entitlement state machine, dispute behavior, reconciliation, tax configuration, and billing observability.

### Phase 4 — Publish / Unpublish API + desktop client

Implement idempotent item upsert/delete, URL validation, history tags, account-switch reconciliation, token storage, retry behavior, and toasts.
...
WAF rollout, rate limits, backup/restore drill, secret rotation drill, accessibility and browser tests, privacy/legal/tax approval, sandbox lifecycle tests, production smoke purchase, rollback drill, support runbooks, and go-live sign-off.
...
3. pgTAP/RLS matrix covers `anon`, user A `aal1`, user A `aal2`, user B `aal2`, active/expired trial, active/lapsed subscription, and removed factor; `aal1` cannot select, insert, update, or delete gallery rows.
4. Owner ID cannot be reassigned; direct Data API access cannot bypass ownership, strong-auth, or entitlement checks.
...
7. Unpublish removes the row and cache; a repeated DELETE succeeds.
...
6. Quarterly restore verifies Auth, RLS, deletion replay, entitlement reconciliation, RPO, and RTO.
7. Secret rotation and known-good Vercel/Cloudflare rollback drills complete without data loss or prolonged billing mismatch; production deployment tests acknowledge immediate alias assignment and trigger automatic rollback on failure.
...
| Cloudflare | DNS-only delivery path; locked R2 operations ledger; Vercel owns proxy/CDN/WAF | Avoids double-proxy loss of visibility while keeping deletion replay outside Supabase PITR |
...
6. **Vercel/Cloudflare readiness:** production domain/TLS, DNS-only Vercel records, DNSSEC/registrar lock, Vercel WAF, log drain/alerts, preview protection, environment isolation, security headers/CSP, kill switches, and a verified rollback.
