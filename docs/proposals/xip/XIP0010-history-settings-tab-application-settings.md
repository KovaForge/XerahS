...
## Legacy content from `XIP0010-history-settings-tab-application-settings.md`
...
## Legacy content from `XIP0010_History_Settings.md`
...
## Deliverables
- Γ£à History tab in ApplicationSettingsView.axaml with full UI
- Γ£à SettingsViewModel.cs has all history-related properties
- Γ£à Settings persist when changed
- Γ£à Build succeeds on `feature/history-settings`
- Γ£à Commit and push changes

## Testing

### Visual Test
1. **Run app** and navigate to Settings ΓåÆ Application Settings ΓåÆ History tab
2. **Verify** all checkboxes and numeric input display correctly
3. **Toggle checkboxes** - confirm they update
4. **Change max count** - verify it accepts values

### Functional Test
1. **Disable "Save to history"** ΓåÆ capture an image ΓåÆ verify it doesn't appear in History view
2. **Enable "Save to history"** ΓåÆ capture an image ΓåÆ verify it appears in History view
3. **Set max recent tasks** to 5 ΓåÆ capture 6 images ΓåÆ verify only 5 appear in recent tasks
4. **Restart app** ΓåÆ verify settings persist
