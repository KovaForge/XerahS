...
## Legacy content from `XIP0012-import-sharex-uploadersconfigjson.md`
...
## Legacy content from `XIP0012_Import_ShareX_UploadersConfig.md`
...
public class ImportResult
{
    ...
    {
        ImportedUploaders.Add(uploaderName);
        DebugHelper.WriteLine($"  Γ£ô Imported: {uploaderName}");
    }
    ...
    public string GetSummary()
    {
        ...
        return $"Successfully imported {TotalImported} uploader(s):\n" + 
               string.Join("\n", ImportedUploaders.Select(u => $"  ΓÇó {u}"));
    }
}
```
...
## Deliverables
- Γ£à `UploadersConfigImporter.cs` with import logic for all uploader categories
- Γ£à `ImportResult` class to track imported uploaders
- Γ£à `ImportShareXConfigCommand` in `DestinationSettingsViewModel`
- Γ£à "Import ShareX UploadersConfig" button in `DestinationSettingsView.axaml`
- Γ£à Build succeeds on `feature/import-uploaders-config`
- Γ£à Manual test: Import real ShareX config successfully
...
2. **Test auto-detection**:
   - Run ShareX Avalonia
   - Navigate to Settings ΓåÆ Destination Settings
   - Click "Import ShareX UploadersConfig" button
   - Verify it finds the config automatically if in default location
...
```
[UploadersConfigImporter] Found ShareX UploadersConfig at: C:\Users\...\Documents\ShareX\UploadersConfig.json
[UploadersConfigImporter]   Γ£ô Imported: Imgur
[UploadersConfigImporter]   Γ£ô Imported: Dropbox
[UploadersConfigImporter]   Γ£ô Imported: Google Drive
[UploadersConfigImporter]   Γ£ô Imported: FTP
[UploadersConfigImporter]   Γ£ô Imported: Custom Uploaders (3)
[UploadersConfigImporter] Import complete: 5 settings imported
```
