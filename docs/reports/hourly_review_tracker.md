...
| Imgur uploader plugin | 2026-04-22 15:49 GMT+8 | Reviewed | Fixed Imgur token hydration so malformed persisted OAuth token secrets are ignored instead of throwing during config load and leaving the uploader UI unusable | Medium | Follow-up review of config/token hydration and uploader auth bootstrap paths; full `dotnet build --configuration Release -m:1 /p:UseSharedCompilation=false /nr:false` passed with 0 warnings/errors, targeted Imgur tests passed 6/6, and full `dotnet test --configuration Release --no-build -m:1 /p:UseSharedCompilation=false /nr:false` passed 430/430. `ShareX.ImageEditor` remained on `develop` at `8bd53991b1173f4ed4698c17cd06cafce81e4cc1`, not detached, with no pointer change required. |
