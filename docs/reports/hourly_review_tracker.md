...
| Plugin loading/runtime | 2026-04-24 18:57 AWST | Reviewed | Fixed plugin manifest path sanitization so malicious `PluginId` or `AssemblyFileName` values can no longer escape the plugin directory during package install/discovery | High | Focused review of plugin package install/discovery and manifest validation across `PluginPackager`, `PluginDiscovery`, `ProviderCatalog`, `PluginLoader`, `PluginFolderCleaner`, and `PluginManifest`; upstream `develop` had 0 pending commits and no conflicts, `ShareX.ImageEditor` remained on branch `develop` at `8bd53991b1173f4ed4698c17cd06cafce81e4cc1` with remotes verified and no pointer update required, `Directory.Build.props` was bumped from `0.22.33` to `0.22.34`, targeted plugin manifest security regressions passed 4/4, full `dotnet build --configuration Release -m:1 /p:UseSharedCompilation=false /nr:false` passed with 0 warnings/errors, and full `dotnet test --configuration Release -m:1 /p:UseSharedCompilation=false /nr:false` passed 516/516. |
