...
| Imgur uploader plugin | 2026-04-23 07:42 AWST | Reviewed | Fixed provider-side Imgur OAuth token hydration so malformed persisted `oauthToken` secrets are ignored instead of throwing during uploader creation or explorer auth bootstrap, which could make saved Imgur configs unusable | Medium | Follow-up review of provider/view-model token hydration symmetry across `ImgurProvider`, `ImgurUploader`, and `ImgurConfigViewModel`; upstream `develop` had 0 pending commits and no conflicts, `ShareX.ImageEditor` remained on `develop` at `8bd53991b1173f4ed4698c17cd06cafce81e4cc1` with no pointer update required, targeted Imgur regressions passed 8/8, full `dotnet build --configuration Release --no-restore -m:1 /p:UseSharedCompilation=false /nr:false` passed with 0 warnings/errors, and full `dotnet test --configuration Release --no-build -m:1 /p:UseSharedCompilation=false /nr:false` passed 457/457. |
...
| Hotkeys/input | 2026-04-23 08:42 AWST | Reviewed | Fixed workflow ID repair on load/save so existing hotkey workflows with a blank or stale `TaskSettings.WorkflowId` are re-synced to the owning workflow ID instead of persisting broken workflow references | Medium | Follow-up review of hotkey workflow ID normalization in persisted config loading, workflow registration, and assistant hotkey handling; upstream `develop` had 0 pending commits and no conflicts, `ShareX.ImageEditor` remained on branch `develop` at `8bd53991b1173f4ed4698c17cd06cafce81e4cc1` with no pointer update required, targeted workflow-config regressions passed 3/3, full `dotnet build --configuration Release --no-restore -m:1 /p:UseSharedCompilation=false /nr:false` passed with 0 warnings/errors, and full `dotnet test --configuration Release --no-build -m:1 /p:UseSharedCompilation=false /nr:false` passed 459/459. |
| Notifications/toasts | 2026-04-23 10:42 AWST | Reviewed | Fixed shared history/toast context-menu visibility so file-dependent actions are hidden when there is no file path or the local file is missing, avoiding broken Open/Upload/Delete/Edit actions on toast entries without a usable backing file | Medium | Follow-up review of shared `HistoryItemMenuFlyout`, toast/history menu-target adapters, and toast file-state gating; upstream `develop` had 0 pending commits and no conflicts, `ShareX.ImageEditor` remained on branch `develop` at `8bd53991b1173f4ed4698c17cd06cafce81e4cc1` with no pointer update required, full `dotnet build --configuration Release --no-restore -m:1 /p:UseSharedCompilation=false /nr:false` passed with 0 warnings/errors, targeted menu-state regressions passed, and full `dotnet test tests/XerahS.Tests/XerahS.Tests.csproj --configuration Release --no-build -m:1 /p:UseSharedCompilation=false /nr:false` passed 460/460. |
