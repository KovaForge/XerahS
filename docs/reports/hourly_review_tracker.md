...
| Settings/configuration | 2026-04-22 12:40 GMT+8 | Reviewed | Fixed provider-context secret-store caching so toggling machine-specific secrets recreates the provider context against the new secrets path instead of reusing a stale store; also repaired a stale region-capture test assertion to match the current `RecordingErrorEventArgs.Error` API so full Release verification runs green again | High | Follow-up review of settings path resolution and provider secret-store wiring; full `dotnet build --configuration Release -m:1` passed with 0 warnings/errors and full `dotnet test --configuration Release --no-build -m:1` passed 428/428. `ShareX.ImageEditor` remained on `develop` at `8bd53991b1173f4ed4698c17cd06cafce81e4cc1`, not detached, with no pointer change required. |
