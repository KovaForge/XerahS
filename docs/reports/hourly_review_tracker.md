...
| Hotkeys/input | 2026-04-22 16:50 GMT+8 | Reviewed | Fixed workflow duplication so cloned workflows rebind `TaskSettings.WorkflowId` to the new workflow identity instead of silently retaining the source workflow ID, which could misroute later workflow lookups and execution state | Medium | Follow-up review of workflow duplication and workflow registration state in the hotkeys/workflow UI area; full `dotnet build --configuration Release --no-restore -m:1 /p:UseSharedCompilation=false /nr:false` passed with 0 warnings/errors, full `dotnet test --configuration Release --no-build -m:1 /p:UseSharedCompilation=false /nr:false` passed 431/431, and `ShareX.ImageEditor` remained on `develop` at `8bd53991b1173f4ed4698c17cd06cafce81e4cc1` with no pointer update required. |
