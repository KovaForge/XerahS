...
| Settings/configuration | 2026-04-23 06:11 AWST | Reviewed | Fixed reset-flow provider-context caching so `SettingsManager.ResetSettings()` now clears the cached provider context before the next secrets-store access, preventing secret operations from reusing a stale context/path after reset | High | Follow-up review of settings reset/path resolution and provider secret-store wiring in `SettingsManager` and `ProviderContextManager`; upstream `develop` had 0 pending commits and no conflicts, `ShareX.ImageEditor` remained on branch `develop` at `8bd53991b1173f4ed4698c17cd06cafce81e4cc1` with no pointer update required, targeted settings-path regressions passed 8/8, full `dotnet build --configuration Release --no-restore -m:1 /p:UseSharedCompilation=false /nr:false` passed with 0 warnings/errors, and full `dotnet test --configuration Release --no-build -m:1 /p:UseSharedCompilation=false /nr:false` passed 454/454. |
