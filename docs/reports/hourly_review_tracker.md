...
| Editor integration | 2026-04-24 16:56 AWST | Reviewed | Fixed history edit-command routing so plain **Edit image** now opens the non-annotation editor path, while **Edit annotations** remains the sidecar-restoring entry point; this also restores history refresh after plain edits because the refresh logic now runs against both editor host paths in tests | High | Follow-up review of `HistoryViewModel` editor command routing and history-refresh behavior across `HistoryEditorLaunchTests`; upstream `develop` had 0 pending commits and no conflicts, `ShareX.ImageEditor` remained on branch `develop` at `8bd53991b1173f4ed4698c17cd06cafce81e4cc1` while upstream/develop was `d285844652e7fa98c24baf7626959927a37762aa`, with no submodule pointer update required; `Directory.Build.props` was bumped from `0.22.31` to `0.22.32`; full `dotnet build --configuration Release -m:1` passed with 0 warnings/errors after avoiding the known parallel Avalonia file-lock race, targeted editor regressions passed 6/6, and full `dotnet test --configuration Release --no-build -m:1` passed 511/511. |
