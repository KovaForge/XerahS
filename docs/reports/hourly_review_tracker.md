...
| Nextcloud uploader plugin | 2026-04-23 00:10 GMT+8 | Reviewed | Fixed Nextcloud config secret hydration so switching to a different saved profile without stored secrets clears stale in-memory app/share passwords instead of silently reusing credentials from the previous profile | Medium | Follow-up review of config view-model/client/uploader state hydration and secret-store loading paths; full `dotnet build --configuration Release -maxcpucount:1` passed with 0 warnings/errors, full `dotnet test --configuration Release --no-build -maxcpucount:1` passed 448/448, and `ShareX.ImageEditor` remained on `develop` at `8bd53991b1173f4ed4698c17cd06cafce81e4cc1` with no pointer update required. |
