...
public partial class VideoConverterWindow : SurfaceWindow
{
    ...
    public VideoConverterViewModel? ViewModel => _viewModel;

    public void Initialize(VideoConverterViewModel viewModel)
    {
    ...
    private async void OnFilePickerRequested(object? sender, EventArgs e)
    {
        var files = await StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions
        {
            ...
            FileTypeFilter = new[]
            {
                new FilePickerFileType("Video files") { Patterns = new[] { "*.mp4", "*.mkv", "*.avi", "*.webm", "*.mov", "*.wmv", "*.flv", "*.gif" } },
