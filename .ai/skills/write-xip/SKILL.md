...
description: How to write effective XerahS Improvement Proposals (XIPs) for new features, architecture changes, and cross-platform implementations. Use for XIP structure, templates, and patterns. For creating/syncing XIPs use the sync-xips skill.
---
...
**Workflow**: **GitHub is the source of truth** for XIPs; the `tasks/` folder is a **backup**. Create and edit XIPs as GitHub issues (label `xip`), then run the sync script to update the tasks folder. See [sync-xips](../sync-xips/SKILL.md) for create/edit/sync workflows.
...
### Create / Edit / Sync
Use the **[sync-xips](../sync-xips/SKILL.md)** skill to create issues, edit them, and run `sync-from-github.ps1` to refresh the tasks backup.
